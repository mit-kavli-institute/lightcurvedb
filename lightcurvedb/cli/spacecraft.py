...
def spacecraft(ctx):
    """
    Subcommand for importing various spacecraft telemetry.
    """
    pass
...
@click.pass_context
@click.argument(
    "ephemris_csv", nargs=-1, type=click.Path(dir_okay=False, exists=True)
)
def ingest(ctx, ephemris_csv):
    ...
    with ctx.obj["dbconf"] as db:
        for i, row in ephemris_df.iterrows():
            eph = SpacecraftEphemris(barycentric_dynamical_time=i, **dict(row))
