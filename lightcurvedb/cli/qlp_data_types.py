...
def create_lightcurvetype(ctx, lightcurve_type_name):
    with ctx.obj['dbconf'] as db:
        check = db.session.query(LightcurveType).filter(
            LightcurveType == lightcurve_type_name
        ).one_or_none()
        if check:
            # Updating
            click.echo(
                click.style('Updating {0}'.format(check), fg='yellow')
            )
            value = click.prompt(
                'Enter a new name '
            ...
            click.echo(
                click.style(
                    'Creating new frame type {0}'.format(
                        lightcurve_type_name
                    ),
                    fg='green'
                )
            )
            desc = click.prompt(
                'Enter a description for {0}'.format(
                    lightcurve_type_name
                )
            )
