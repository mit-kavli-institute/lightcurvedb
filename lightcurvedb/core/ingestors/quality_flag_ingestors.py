...
def load_qflag_file(
    db_session, qflag_file, camera_override=None, ccd_override=None
):
    ...
    if camera_override:
        ...
        camera = re.search(r"cam(?P<camera>[1-4])", base_file).groupdict()[
            "camera"
        ]
    ...
    if ccd_override:
        ...
        ccd = re.search(r"ccd(?P<ccd>[1-4])", base_file).groupdict()["ccd"]
    ...
    try:
        ...
        q_df["camera"] = camera
        q_df["ccd"] = ccd
        db_session.bulk_insert_mappings(QualityFlags, q_df.to_dict("records"))
    except FileNotFoundError:
        logger.debug(
            "Could not find specified quality flag file {0}".format(qflag_file)
        )
    except OSError:
        logger.debug(
            "Could not open specified quality flag file {0}".format(qflag_file)
        )
...
def get_qflag_df(db_session):
    ...
    df = pd.read_sql(
        q.statement, db_session.bind, index_col=["cadences", "camera", "ccd"]
    )
    return df
...
def update_qflag(quality_flag_df, lp_df):
    ...
    lp_df.reset_index(inplace=True)
    lp_df.set_index(["lightcurve_id", "cadences"], inplace=True)
