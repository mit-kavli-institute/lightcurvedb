...
def test_orbit_cadence_limit(db_conn, frames):
    try:
        ...
        note(db_conn.query(func.max(Frame.cadence), func.min(Frame.cadence)).all())
        assert min_c == min([f.cadence for f in frames])
        assert max_c == max([f.cadence for f in frames])
...
def test_orbit_tjd_limit(db_conn, frames):
    try:
        ...
        assert min_tjd == min(f.start_tjd for f in frames)
        assert max_tjd == max(f.end_tjd for f in frames)
        db_conn.session.rollback()
...
def test_get_frames_by_orbit(db_conn, frames):
    try:
        ...
        q = QlpQuery(conn=db_conn)
        frames_result = q.query_orbit_tjd_limit(orbit.orbit_number, 30, 1)
        note(frames_result)
        assert all(frames[0].orbit.orbit_number == f[0] for f in frames_result)
