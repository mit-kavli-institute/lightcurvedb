...
PSQL_INT = st.integers(
    min_value=-1*PSQL_INT_MAX,
    max_value=PSQL_INT_MAX
)

DEFINED_FLOAT = st.floats(
...
def orbits(**overrides):
    return st.builds(
        ...
        crm_n=overrides.get('crm_n', st.integers(min_value=0, max_value=PSQL_INT_MAX)),
        crm=overrides.get('crm', st.booleans()),
        basename=overrides.get("basename", postgres_text(min_size=1, max_size=64))
    )
...
def frames(**overrides):
    return st.builds(
        models.Frame,
        cadence_type=overrides.get('cadence_type', st.integers(min_value=1, max_value=32767)),
        camera=overrides.get('camera', st.integers(min_value=1, max_value=4)),
        ccd=overrides.get('ccd', st.integers(min_value=1, max_value=4)),
        cadence=overrides.get('cadence', st.integers(min_value=0, max_value=PSQL_INT_MAX)),
