...
def upgrade():
    ...
    qlpproc_id_seq = sa.Sequence('qlpprocess_id_seq', cache=100)
    qlpalt_id_seq = sa.Sequence('qlpalteration_id_seq', cache=100)
    op.execute(sa.schema.CreateSequence(qlpproc_id_seq))
    op.execute(sa.schema.CreateSequence(qlpalt_id_seq))

    op.create_table('qlpprocesses',
    sa.Column('created_on', sa.DateTime(), server_default=sa.text(u'now()'), nullable=True),
    sa.Column('id', sa.Integer(), qlpproc_id_seq, nullable=False),
    ...
    sa.Column('created_on', sa.DateTime(), server_default=sa.text(u'now()'), nullable=True),
    sa.Column('id', sa.BigInteger(), qlpalt_id_seq, nullable=False),
...
def downgrade():
    ...
    op.execute('DROP SEQUENCE qlpprocess_id_seq')
    op.execute('DROP SEQUENCE qlpalteration_id_seq')
    # ### end Alembic commands ###
