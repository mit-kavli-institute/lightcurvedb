...
def upgrade():
    ...
    op.create_table('camera_quaternions',
    ...
    sa.CheckConstraint(u'camera IN (1, 2, 3, 4)', name='phys_camera_constraint'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('camera', 'date')
    )
    op.create_index(op.f('ix_camera_quaternions_camera'), 'camera_quaternions', ['camera'], unique=False)
    op.create_index(op.f('ix_camera_quaternions_date'), 'camera_quaternions', ['date'], unique=False)
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_camera_quaternions_date'), table_name='camera_quaternions')
    op.drop_index(op.f('ix_camera_quaternions_camera'), table_name='camera_quaternions')
